...
class PolicyNormProcessor:
    ...
    # ------------------------------------------------------------------
    # Forward path (env state -> model input state)
    # ------------------------------------------------------------------
    def apply_state(self, raw_state: np.ndarray) -> np.ndarray:
        """Normalize raw env state using the training-time pipeline.
        ...
        Returns:
            ``(T, D)`` normalized state in the same concatenated key order used
        ...
        if not self._state_keys:
            raise ValueError("Checkpoint data config has no state_keys; cannot normalize state for inference.")

        data: Dict[str, np.ndarray] = {}
        cursor = 0
        for full_key in self._state_keys:
            dim_k = self._state_key_dims.get(full_key, 1)
            data[full_key] = raw_state[..., cursor : cursor + dim_k]
            cursor += dim_k

        if cursor != raw_state.shape[-1]:
            raise ValueError(
                f"Sum of per-key state dims ({cursor}) != state_dim "
                f"({raw_state.shape[-1]}). state_keys={self._state_keys}, "
