#!/usr/bin/env bash
# Final AWAC phase 2: train actor with frozen actor_pi snapshot and frozen Q critic.
set -Eeuo pipefail
trap 'echo "[final-awac-actor] FAILED at line ${LINENO}" >&2' ERR
...
CRITIC_RUN_ID=${CRITIC_RUN_ID:-awac_critic_pi_lora_sem_qwen35_4b_bc20k_onfly}
AWAC_ROOT=${AWAC_ROOT:-${PROJECT_ROOT}/logs/final_awac_pi_lora_sem_onfly}
...
critic_checkpoint=${CRITIC_CHECKPOINT:-${critic_checkpoint:-${AWAC_ROOT}/${CRITIC_RUN_ID}/checkpoints/steps_50000_critic.pt}}
run_root_dir=${RUN_ROOT_DIR:-${run_root_dir:-${AWAC_ROOT}}}
run_id=${RUN_ID:-${run_id:-awac_actor_pi_lora_sem_qwen35_4b_bc20k_onfly}}
ALLOW_EXISTING_RUN_DIR=${ALLOW_EXISTING_RUN_DIR:-false}

if [[ -e "${run_root_dir}/${run_id}" && "${ALLOW_EXISTING_RUN_DIR}" != "true" ]]; then
  echo "[final-awac-actor] ERROR: run directory already exists: ${run_root_dir}/${run_id}" >&2
  echo "[final-awac-actor] Use a new RUN_ID or set ALLOW_EXISTING_RUN_DIR=true only when resuming intentionally." >&2
  exit 2
fi
...
test -f "${PROJECT_ROOT}/examples/calvin/train_files/run_calvin_awac_actor.sh"
...
export main_process_port=${main_process_port:-29640}
...
echo "[final-awac-actor] run_dir=${run_root_dir}/${run_id}"
echo "[final-awac-actor] base_vlm=${base_vlm}"
echo "[final-awac-actor] bc_checkpoint=${bc_checkpoint}"
echo "[final-awac-actor] critic_checkpoint=${critic_checkpoint}"
echo "[final-awac-actor] data=${calvin_data_root}/${calvin_dataset_name}"
echo "[final-awac-actor] compute_rewards_on_the_fly=${compute_rewards_on_the_fly} assume_success_if_missing=${assume_success_if_missing}"

bash examples/calvin/train_files/run_calvin_awac_actor.sh

FINAL_ACTOR_CKPT="${run_root_dir}/${run_id}/checkpoints/steps_${actor_max_train_steps}_pytorch_model.pt"
test -f "${FINAL_ACTOR_CKPT}"

echo "[final-awac-actor] OK"
echo "FINAL_ACTOR_CKPT=${FINAL_ACTOR_CKPT}"
