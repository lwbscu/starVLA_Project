...
if [[ "${ADAPTER_HIDDEN_DIM}" == "auto" ]]; then
  ADAPTER_HIDDEN_DIM="${QWEN_VL_HIDDEN_DIM}"
fi
...
if [[ -n "${CHECKPOINT_KEEP_LATEST}" ]]; then
  COMMON_ARGS+=(--trainer.checkpoint_keep_latest "${CHECKPOINT_KEEP_LATEST}")
fi

if [[ -n "${CHECKPOINT_KEEP_STEPS}" ]]; then
  COMMON_ARGS+=(--trainer.checkpoint_keep_steps "${CHECKPOINT_KEEP_STEPS}")
fi
